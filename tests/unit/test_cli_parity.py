...
def test_check_parses_every_new_flag():
    ...
    assert args.ai_detection_device == "cuda"
...
def test_check_defaults_mirror_web():
    ...
    assert args.ai_detection_device == "cpu"
...
def test_ai_detection_device_propagates_to_checker(monkeypatch, tmp_path):
    _patch_checker_and_analyses(monkeypatch)
    bib = tmp_path / "refs.bib"
    bib.write_text("@article{a, title={X}}", encoding="utf-8")

    args = cli.parse_args([
        "check", "--paper", str(bib), "--ai-detection", "local",
        "--ai-detection-consent", "--ai-detection-device", "cuda", "--json",
    ])
    with redirect_stdout(io.StringIO()):
        cli.run_check(args)

    assert _FakeChecker.last_kwargs["ai_detection_device"] == "cuda"
