...
def deps_available() -> bool:
    """Whether the installed weights have a compatible inference runtime.

    Make an on-demand ``--target`` runtime importable first, then match the
    runtime to the actual weight format. ONNX Runtime cannot load the managed
    safetensors checkpoint, and torch cannot load a standalone ONNX graph.
    """
    ...
    # The managed model download is safetensors-only.
    return _torch_deps_available()
