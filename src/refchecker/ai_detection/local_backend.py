...
# Module-level model cache keyed by detector and device. Keeping CPU and CUDA
# entries separate prevents the first request's device from silently
# determining all later checks, while also supporting upstream's detector
# roster without loading the same weights twice.
...
def _engine_cache_key(detector_key: str, device: str) -> str:
    return f"{detector_key}:{_normalize_device(device)}"
    ...
    def detect(self, text: str, *, title: Optional[str] = None) -> AIDetectionResult:
        ...
        try:
            ...
            try:
                ...
                pass
            result = make_unavailable("model_load_failed", self.name, wc, detail=detail)
            result.device_used = self.device
            return result
        ...
        if not kept:
            result = make_inconclusive("insufficient_signal", self.name, wc)
            result.device_used = self.device
            return result
        ...
        try:
            ...
            result = make_unavailable("inference_failed", self.name, wc, detail=detail)
            result.device_used = self.device
            return result
...
def _build_engine_at(model_dir_path, head: Optional[str] = None, device: str = "cpu"):
...
def _get_engine(device: str = "cpu"):
    ...
    cache_key = _engine_cache_key(model_manager.DEFAULT_DETECTOR, device)
    ...
    with _engine_lock:
        ...
        build_kwargs = {"head": "custom_mean_pool"}
        if device != "cpu":
            build_kwargs["device"] = device
        built = _build_engine_at(model_manager.model_path(), **build_kwargs)
        _engines[cache_key] = built
        return built


def load(detector_key: str, device: str = "cpu"):
    ...
    if key == model_manager.DEFAULT_DETECTOR:
        return _get_engine(device)
    cache_key = _engine_cache_key(key, device)
    cached = _engines.get(cache_key)
    if cached is not None:
        return cached
    with _engine_lock:
        ...
        try:
            ...
            logger.debug("ensure_on_path before engine load failed: %s", exc)
        build_kwargs = {"head": str(entry.get("head"))}
        if device != "cpu":
            build_kwargs["device"] = device
        built = _build_engine_at(model_manager.detector_dir(key), **build_kwargs)
        _engines[cache_key] = built
        return built
    ...
    if os.path.isfile(st):
        ...
        for fname in ("pytorch_model.bin", "model.bin"):
            ...
            if os.path.isfile(binp):
                import torch
                state_dict = torch.load(binp, map_location="cpu")
                break
    if state_dict is None:
        return None
    # Some desklib revisions store encoder tensors under ``deberta.*`` while
    # our lightweight wrapper exposes the encoder as ``model.*``.
    return {
        ("model." + key[len("deberta."):]) if key.startswith("deberta.") else key: value
...
class _OnnxEngine:
    ...
    def __init__(self, model_dir: str, onnx_path: str, device: str = "cpu"):
        ...
        with warnings.catch_warnings():
            ...
            self.tokenizer = AutoTokenizer.from_pretrained(model_dir, local_files_only=True)
        if _normalize_device(device) == "cuda":
