...
def _run_one(
    ...
    try:
        ...
        engine = (
            local_backend.load(key)
            if (device or "cpu").lower() == "cpu"
            else local_backend.load(key, device=device)
        )
...
def run_detectors(text_or_pages, keys: Sequence[str], device: str = "cpu") -> Dict:
    ...
    for key in ordered_keys:
        if abstain_reason:
            ...
            results.append(_run_one(key, body, wc, sentences, device=device))
