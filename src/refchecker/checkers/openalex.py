...
class OpenAlexReferenceChecker:
    ...
    def search_works(self, query: str, year: Optional[int] = None, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        hit = cached_api_response(getattr(self, 'cache_dir', None), 'openalex', 'search_works', cache_q)
        if hit is not None:
            return hit
        result = self._search_works_uncached(query, year, limit)
        cache_api_response(getattr(self, 'cache_dir', None), 'openalex', 'search_works', cache_q, result)
        return result
    ...
    def get_work_by_doi(self, doi: str) -> Optional[Dict[str, Any]]:
        from refchecker.utils.cache_utils import cached_api_response, cache_api_response
        hit = cached_api_response(getattr(self, 'cache_dir', None), 'openalex', 'get_by_doi', doi)
        if hit is not None:
            return hit
        result = self._get_work_by_doi_uncached(doi)
        cache_api_response(getattr(self, 'cache_dir', None), 'openalex', 'get_by_doi', doi, result)
        return result
