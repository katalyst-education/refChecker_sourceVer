...
import csv
import subprocess
from service_core.core.hallucination_policy import should_check_hallucination, assess_hallucination
from service_core.core.report_builder import ReportBuilder
from service_core.checkers.local_semantic_scholar import LocalNonArxivReferenceChecker
from service_core.utils.text_utils import (clean_author_name, clean_title, clean_title_basic,
                       ...
                       compare_authors)
from service_core.utils.url_utils import extract_arxiv_id_from_url, construct_semantic_scholar_url
from service_core.utils.database_config import resolve_database_paths, resolve_database_update_paths, DATABASE_LABELS, DATABASE_UPDATE_ORDER
from service_core.utils.config_validator import ConfigValidator
from service_core.services.pdf_processor import PDFProcessor
from service_core.checkers.enhanced_hybrid_checker import EnhancedHybridReferenceChecker
from service_core.core.parallel_processor import ParallelReferenceProcessor  
from service_core.core.db_connection_pool import ThreadSafeLocalChecker
from service_core.database.local_database_updater import update_local_database

# Import version
from service_core.__version__ import __version__
from service_core.llm.base import create_llm_provider, ReferenceExtractor
...
def get_llm_api_key_interactive(provider: str) -> str:
    ...
    """
    from service_core.config.settings import _PROVIDER_ENV_VARS, resolve_api_key
...
def prepare_openreview_paper_specs(venue_spec, output_dir='output', status='accepted', output_path=None):
    ...
    from service_core.checkers.openreview_checker import OpenReviewReferenceChecker
...
class ArxivReferenceChecker:
    def __init__(self, semantic_scholar_api_key=None, db_path=None, output_file=None,
        ...
        if not llm_disabled:
            from service_core.config.settings import HALLUCINATION_CAPABLE_PROVIDERS
            try:
                from service_core.llm.hallucination_verifier import LLMHallucinationVerifier
        ...
        try:
            from service_core.checkers.web_search import create_web_search_checker
    ...
    def extract_arxiv_id_from_url(self, url):
        ...
        Uses the common extraction function from service_core.utils.url_utils.
        """
    ...
    def get_paper_metadata_from_semantic_scholar(self, arxiv_id):
        ...
        try:
            import requests
            from service_core.utils.cache_utils import cached_api_response, cache_api_response
    ...
    def download_pdf(self, paper):
        ...
        # Check PDF cache
        from service_core.utils.cache_utils import cached_pdf, cache_pdf
    ...
    def download_pdf_from_url(self, url):
        ...
        """
        from service_core.utils.url_utils import download_pdf_bytes
        self.last_download_error = None
        try:
    ...
    def extract_authors_list(self, authors_text):
        ...
        if len(and_parts) > 1:
            ...
            from service_core.utils.text_utils import parse_authors_with_initials
    ...
    def verify_db_reference(self, source_paper, reference, db_conn):
        ...
        if title and paper_data.get('title'):
            ...
            if normalized_title != db_title:
                from service_core.utils.error_utils import format_title_mismatch
        ...
        from service_core.utils.error_utils import validate_year
        ...
        if doi and external_ids.get('DOI'):
            from service_core.utils.doi_utils import compare_dois, normalize_doi
            ...
            if not compare_dois(doi, external_ids['DOI']):
                ...
                if not actual_doi_normalized.startswith(cited_doi_normalized.rstrip('.')):
                    ...
                    from service_core.utils.error_utils import format_doi_mismatch
                    from service_core.utils.doi_utils import validate_doi_resolves
    ...
    def _try_arxiv_re_verify(self, errors, paper_url, verified_data, reference):
        ...
        from service_core.core.hallucination_policy import _compute_author_overlap
        ...
        arxiv_id = None
        try:
            from service_core.checkers.arxiv_citation import ArXivCitationChecker
    ...
    def verify_github_reference(self, reference):
        ...
        from service_core.checkers.github_checker import GitHubChecker
    ...
    def verify_webpage_reference(self, reference):
        ...
        from service_core.checkers.webpage_checker import WebPageChecker
    ...
    def verify_raw_url_reference(self, reference):
        ...
        from service_core.checkers.pdf_paper_checker import PDFPaperChecker
    ...
    def add_error_to_dataset(self, source_paper, reference, errors, reference_url=None, verified_data=None):
        ...
        if len(errors) > 1:
            ...
            from service_core.utils.text_utils import format_corrected_plaintext, format_corrected_bibtex, format_corrected_bibitem
    ...
    def run(self, debug_mode=False, specific_paper_id=None, local_pdf_path=None, input_specs=None):
        ...
        try:
            ...
            if len(raw_input_specs) > 1:
                from service_core.core.bulk_pipeline import run_bulk_paper_check
            ...
            for paper in paper_iterator:
                ...
                try:
                    ...
                    from service_core.utils.cache_utils import cache_bibliography, llm_cache_identity_from_extractor
    ...
    def _parse_acm_reference_format(self, ref, content):
        ...
        if author_field_match:
            ...
            from service_core.utils.text_utils import extract_bibinfo_person_content
        ...
        from service_core.utils.text_utils import extract_bibinfo_field_content
    ...
    def _parse_references_regex(self, bibliography_text):
        ...
        from service_core.utils.bibtex_parser import detect_bibtex_format
        ...
        from service_core.utils.biblatex_parser import detect_biblatex_format  
            ...
            if arxiv_url:
                ...
                for pattern in doi_patterns:
                    ...
                    if doi_match:
                        ...
                        if doi:
                            from service_core.utils.doi_utils import construct_doi_url
    ...
    def _parse_bibtex_references(self, bibliography_text):
        ...
        from service_core.utils.bibtex_parser import parse_bibtex_references
    ...
    def _parse_biblatex_references(self, bibliography_text):
        ...
        from service_core.utils.biblatex_parser import parse_biblatex_references
    ...
    def _clean_llm_author_text(self, author_text):
        ...
        if '*' in author_text:
            ...
            for author in raw_authors:
                ...
                from service_core.utils.text_utils import strip_latex_commands
    ...
    def _create_structured_llm_references(self, ref_text):
        ...
        if not url and not arxiv_url:
            ...
            if url_match:
                from service_core.utils.url_utils import clean_url_punctuation
    ...
    def extract_bibliography(self, paper, debug_mode=False, input_spec=None):
        ...
        from service_core.utils.cache_utils import cached_bibliography, llm_cache_identity_from_extractor
        llm_cache_identity = llm_cache_identity_from_extractor(self.llm_extractor)
        ...
        if bibtex_content:
            ...
            if '\\begin{thebibliography}' in bibtex_content and '\\bibitem' in bibtex_content:
                ...
                from service_core.utils.text_utils import validate_parsed_references
    ...
    def _process_reference_result(self, paper, reference, errors, reference_url, 
        ...
        if errors:
            from service_core.core.hallucination_policy import apply_hallucination_verdict
            ...
            from service_core.core.hallucination_policy import count_raw_errors
    ...
    def _get_verified_url(self, verified_data, reference_url, errors):
        ...
        if verified_data and verified_data.get('externalIds', {}).get('ArXiv'):
            ...
            if not self._has_arxiv_id_error(errors):
                from service_core.utils.url_utils import construct_arxiv_url
                correct_arxiv_id = verified_data['externalIds']['ArXiv']
    ...
    def _validate_reference_url(self, reference_url, verified_data):
        ...
        if 'arxiv.org' in reference_url and verified_data:
            ...
            if external_ids.get('ArXiv'):
                ...
                from service_core.utils.url_utils import extract_arxiv_id_from_url
    ...
    def _print_reference_header(self, reference, index, total):
        ...
        """
        from service_core.utils.text_utils import strip_latex_commands, format_authors_for_display
    ...
    def _display_non_unverified_errors(self, errors, debug_mode, print_output):
        """Display all non-unverified errors and warnings"""
        if not debug_mode and print_output:
            from service_core.utils.error_utils import print_labeled_multiline, sort_issues_for_cli_display
    ...
    def _run_and_display_hallucination_assessment(
        ...
        """
        from service_core.core.hallucination_policy import (
